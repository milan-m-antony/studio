...
const getLucideIcon = (iconName?: string | null): ComponentType<LucideIcons.LucideProps> => {
  if (iconName && LucideIcons[iconName as keyof typeof LucideIcons]) {
    const IconComponent = LucideIcons[iconName as keyof typeof LucideIcons] as ComponentType<LucideIcons.LucideProps>;
    if (typeof IconComponent === 'function') { // Check if it's a component
      return IconComponent;
    }
  }
  // console.warn(`HeroSection: Lucide icon "${iconName}" not found or invalid. Falling back to GenericLinkIcon.`);
...
export default function HeroSection({ heroContent }: HeroSectionProps) {
  ...
  }, []); // Removed heroContent from dependency array to prevent re-logging on every minor data change if not needed for scroll effect re-init
  ...
  const socialLinksToRender: Array<{ href: string; iconImageUrl: string | null; label: string; IconComponent: ComponentType<LucideIcons.LucideProps> }> =
    ...
    ? heroContent.social_media_links.map((link, index) => {
        ...
        return {
          ...
          label: link.label,
          IconComponent: GenericLinkIcon // Fallback icon if image URL fails or is not provided
        };
      })
    : [ // Default links if nothing is fetched
        { href: "https://github.com", iconImageUrl: null, label: "GitHub (Default)", IconComponent: LucideIcons.Github },
        { href: "https://linkedin.com", iconImageUrl: null, label: "LinkedIn (Default)", IconComponent: LucideIcons.Linkedin },
      ];
  ...
  return (
    <section id="hero" className="relative h-screen flex flex-col items-center justify-center overflow-hidden text-center bg-background text-foreground p-4">
      ...
      {socialLinksToRender.length > 0 && (
        ...
        >
          {socialLinksToRender.map((social) => {
            const ActualIcon = social.IconComponent; // This is a fallback, image URL takes precedence
            return (
              <NextLink key={social.label} href={social.href} target="_blank" rel="noopener noreferrer" aria-label={social.label}>
                {social.iconImageUrl ? (
                  ...
                  <ActualIcon className="h-5 w-5 sm:h-6 sm:w-6 text-foreground/70 hover:text-primary transition-colors duration-300 ease-in-out transform hover:scale-110" />
                )}
              </NextLink>
            );
          })}
      ...
      <div className="relative z-10 flex flex-col items-center" style={{ transform: `translateY(${offsetY * 0.15}px)` }}>
        <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold mb-4 animate-fadeIn" style={{animationDelay: '0.5s'}}>
          Hi, I'm {mainName}
        </h1>
        <p className="text-2xl sm:text-3xl md:text-4xl font-light mb-16 sm:mb-12 text-foreground/90 min-h-[2.5em] sm:min-h-[1.5em] animate-fadeIn" style={{animationDelay: '0.8s'}}>
          <EnhancedTypewriter
            texts={subtitles}
          />
        </p>
      ...
      <div
        className="absolute bottom-6 sm:bottom-8 md:bottom-10 left-1/2 -translate-x-1/2 z-20 animate-fadeIn"
