...
const DefaultTimelineSvgFallback = (props: React.SVGProps<SVGSVGElement>) => (
  <svg
    ...
    width="24" // Base SVG width, actual size controlled by className
    height="24" // Base SVG height
    viewBox="0 0 24 24"
    fill="none"
    stroke="currentColor"
    strokeWidth="2"
...
export default function TimelineItem({ event, isLeft }: TimelineItemProps) {
    ...
    IconContent = (
      <div className="relative h-full w-full">
        <NextImage
          ...
          layout="fill"
          objectFit="contain"
          className="rounded-full" // If the image itself should also try to be circular
        />
      </div>
    );
  } else {
    IconContent = <DefaultTimelineSvgFallback className="h-8 w-8" />; // Increased size
  }
  ...
  return (
    ...
    >
      ...
      <div className="z-20 flex items-center order-1 shadow-xl w-14 h-14 rounded-full"> {/* Increased size */}
        <div className={cn(
            "mx-auto rounded-full w-14 h-14 flex items-center justify-center overflow-hidden", // Increased size
