...
import { Users as HeroIcon, PlusCircle, Edit, Trash2, Link as LinkIconImg, Image as ImageIcon } from 'lucide-react';
...
export default function HeroManager() {
  ...
  const watchedIconUrlInModal = socialLinkForm.watch("icon_image_url");

  useEffect(() => {
    if (watchedIconUrlInModal) {
      setCurrentIconPreview(watchedIconUrlInModal);
    } else {
      setCurrentIconPreview(null);
    }
  }, [watchedIconUrlInModal]);
  ...
  const onHeroSubmit: SubmitHandler<HeroContentFormData> = async (formData) => {
    ...
    // Prepare social_media_links for Supabase (remove client-side id)
              ...
              {socialLinkForm.formState.errors.icon_image_url && <p className="text-destructive text-sm mt-1">{socialLinkForm.formState.errors.icon_image_url.message}</p>}
               {currentIconPreview && (
                <div className="mt-2 p-1 border rounded-md bg-muted aspect-square relative w-16 h-16">
                  <NextImage src={currentIconPreview} alt="Icon preview" fill className="object-contain rounded"/>
                </div>
              )}
            </div>
            </div></ScrollArea>
