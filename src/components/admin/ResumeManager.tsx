...
import { PlusCircle, Edit, Trash2, Briefcase, GraduationCap, ListChecks, Languages as LanguagesIcon, FileText as ResumeIcon, UploadCloud, Link as LinkIcon, ImageIcon, Building, HelpCircle, ChevronDown, Tag as TagIcon } from 'lucide-react';
...
const IconPreview = ({ url, alt = "Icon Preview", DefaultIcon = HelpCircle }: { url?: string | null; alt?: string, DefaultIcon?: React.ElementType }) => {
  if (!url || typeof url !== 'string' || url.trim() === '') {
    return <DefaultIcon className="h-8 w-8 text-muted-foreground border rounded p-1" />;
  }
...
export default function ResumeManager() {
  ...
  const [parentKeySkillCategoryId, setParentKeySkillCategoryId] = React.useState<string | null>(null);
  ...
  return (
    <>
      <Card className="shadow-lg mb-8">
        <CardHeader><CardTitle className="flex items-center justify-between">Manage General Resume Info <ResumeIcon className="h-6 w-6 text-primary" /></CardTitle><CardDescription>Update the overall resume description and downloadable PDF file.</CardDescription></CardHeader>
        <CardContent>{isLoadingResumeMeta ? <p className="text-center text-muted-foreground">Loading resume info...</p> : (
          <form onSubmit={resumeMetaForm.handleSubmit(onResumeMetaSubmit)} className="grid gap-6 py-4">
            ...
            <div className="space-y-2"><Label htmlFor="resume_pdf_file">Resume PDF File</Label><div className="flex items-center gap-3"><Input id="resume_pdf_file" type="file" accept=".pdf" onChange={handleResumePdfFileChange} className="flex-grow"/><UploadCloud className="h-6 w-6 text-muted-foreground"/></div>{resumePdfFile && <p className="text-sm text-muted-foreground mt-1">New file: {resumePdfFile.name}</p>}{currentDbResumePdfUrl && !resumePdfFile && (<div className="mt-2 text-sm">Current PDF: <a href={currentDbResumePdfUrl} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline inline-flex items-center"><LinkIcon className="mr-1 h-4 w-4" />View</a></div>)}<div><Label htmlFor="resume_pdf_url_manual" className="text-xs text-muted-foreground">Or enter direct PDF URL (upload overrides)</Label><Input id="resume_pdf_url_manual" {...resumeMetaForm.register("resume_pdf_url")} placeholder="https://example.com/resume.pdf" />{resumeMetaForm.formState.errors.resume_pdf_url && <p className="text-destructive text-sm mt-1">{resumeMetaForm.formState.errors.resume_pdf_url.message}</p>}</div></div>
        ...
        <CardContent>
          <Tabs defaultValue="experience" className="w-full">
            <TabsList className="grid w-full grid-cols-2 md:grid-cols-4 mb-6">
              ...
              <TabsTrigger value="languages"><LanguagesIcon className="mr-2 h-4 w-4" />Languages</TabsTrigger>
            </TabsList>

            {/* Experience Tab Content */}
            <TabsContent value="experience">
              <div className="text-right mb-4"><Button onClick={() => handleOpenExperienceModal()}><PlusCircle className="mr-2 h-4 w-4" /> Add Experience</Button></div>
              {isLoadingExperiences ? <p className="text-center text-muted-foreground">Loading experiences...</p> : experiences.length === 0 ? <p className="text-muted-foreground text-center py-4">No experience entries yet.</p> : (
                <div className="space-y-4">{experiences.map((exp) => (<Card key={exp.id} className="p-4"><div className="flex justify-between items-start gap-4"><div className="flex items-center gap-3 flex-grow min-w-0"><IconPreview url={exp.icon_image_url} alt={exp.job_title} DefaultIcon={Briefcase} /><div className="flex-grow min-w-0"><h4 className="font-semibold text-lg truncate" title={exp.job_title}>{exp.job_title}</h4><p className="text-sm text-muted-foreground truncate" title={`${exp.company_name} (${exp.date_range || ''})`}>{exp.company_name} ({exp.date_range || 'Date N/A'})</p></div></div><div className="flex space-x-2 flex-shrink-0"><Button variant="outline" size="sm" onClick={() => handleOpenExperienceModal(exp)}><Edit className="mr-1 h-3 w-3" />Edit</Button><Button variant="destructive" size="sm" onClick={() => triggerExperienceDeleteConfirmation(exp)}><Trash2 className="mr-1 h-3 w-3" />Delete</Button></div></div></Card>))}</div>)}
            </TabsContent>
            
            {/* Education Tab Content */}
            <TabsContent value="education">
              <div className="text-right mb-4"><Button onClick={() => handleOpenEducationModal()}><PlusCircle className="mr-2 h-4 w-4" /> Add Education</Button></div>
                    ...
                    <AccordionItem value={category.id} key={category.id}>
                        <AccordionPrimitive.Header className="flex items-center justify-between py-2 px-4 group border-b hover:bg-muted/50 transition-colors">
                            <AccordionPrimitive.Trigger asChild className="flex-grow cursor-pointer py-2">
                                <div className="flex items-center gap-3"><IconPreview url={category.icon_image_url} alt={category.category_name} DefaultIcon={TagIcon}/><span className="font-medium text-lg">{category.category_name}</span><Badge variant="outline">{category.resume_key_skills?.length || 0} skills</Badge><ChevronDown className="h-4 w-4 shrink-0 transition-transform duration-200 group-data-[state=open]:rotate-180 ml-auto text-muted-foreground group-hover:text-foreground" /></div>
                        ...
                        <AccordionContent className="bg-muted/20 p-4 rounded-b-md">
                            {category.resume_key_skills && category.resume_key_skills.length > 0 ? (<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">{category.resume_key_skills.map(skill => (<Card key={skill.id} className="p-3 shadow-sm hover:shadow-md transition-shadow"><div className="flex items-center justify-between"><span className="text-sm font-medium">{skill.skill_name}</span><div className="flex space-x-1"><Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => handleOpenSkillModal(category.id, skill)}><Edit className="h-3.5 w-3.5" /></Button><Button variant="ghost" size="icon" className="h-7 w-7 hover:text-destructive" onClick={() => triggerSkillDeleteConfirmation(skill)}><Trash2 className="h-3.5 w-3.5" /></Button></div></div></Card>))}</div>) : (<p className="text-sm text-muted-foreground text-center py-4">No skills in this category. <Button variant="link" size="sm" className="p-0 h-auto" onClick={() => handleOpenSkillModal(category.id)}>Add one?</Button></p>)}
                        </AccordionContent>
                    </AccordionItem>
                    ))}
                </Accordion>
            ...
            <TabsContent value="languages">
                <div className="text-right mb-4"><Button onClick={() => handleOpenLanguageModal()}><PlusCircle className="mr-2 h-4 w-4" /> Add Language</Button></div>
                ...
                <div className="space-y-4">{languages.map((lang) => (<Card key={lang.id} className="p-4"><div className="flex justify-between items-start gap-4"><div className="flex items-center gap-3 flex-grow min-w-0"><IconPreview url={lang.icon_image_url} alt={lang.language_name} DefaultIcon={LanguagesIcon}/><div className="flex-grow min-w-0"><h4 className="font-semibold text-lg truncate" title={lang.language_name}>{lang.language_name}</h4><p className="text-sm text-muted-foreground truncate" title={lang.proficiency || ''}>{lang.proficiency || 'Proficiency N/A'}</p></div></div><div className="flex space-x-2 flex-shrink-0"><Button variant="outline" size="sm" onClick={() => handleOpenLanguageModal(lang)}><Edit className="mr-1 h-3 w-3" />Edit</Button><Button variant="destructive" size="sm" onClick={() => triggerLanguageDeleteConfirmation(lang)}><Trash2 className="mr-1 h-3 w-3" />Delete</Button></div></div></Card>))}</div>)}
            </TabsContent>
          </Tabs>
        </CardContent>
      </Card>
        ...
        <DialogContent className="sm:max-w-[525px]"><DialogHeader><DialogTitle>{currentExperience ? 'Edit Experience' : 'Add New Experience'}</DialogTitle></DialogHeader>
          <form onSubmit={experienceForm.handleSubmit(onExperienceSubmit)} className="grid gap-4 py-4">
            <ScrollArea className="max-h-[70vh] p-1"><div className="grid gap-4 p-3">
              ...
              <div><Label htmlFor="exp_date_range">Date Range</Label><Input id="exp_date_range" {...experienceForm.register("date_range")} /></div>
              ...
              <div><Label htmlFor="exp_icon_image_url">Icon Image URL (Optional)</Label><Input id="exp_icon_image_url" {...experienceForm.register("icon_image_url")} placeholder="https://..." />{experienceForm.formState.errors.icon_image_url && <p className="text-destructive text-sm mt-1">{experienceForm.formState.errors.icon_image_url.message}</p>}{experienceForm.watch("icon_image_url") && (<div className="mt-2 flex items-center gap-2"><span className="text-xs">Preview:</span><IconPreview url={experienceForm.watch("icon_image_url")} DefaultIcon={Briefcase}/></div>)}</div>
      ...
      <Dialog open={isSkillCategoryModalOpen} onOpenChange={(isOpen) => { setIsSkillCategoryModalOpen(isOpen); if (!isOpen) setCurrentSkillCategory(null); }}><DialogContent className="sm:max-w-[425px]"><DialogHeader><DialogTitle>{currentSkillCategory ? 'Edit Skill Category' : 'Add Skill Category'}</DialogTitle></DialogHeader><form onSubmit={skillCategoryForm.handleSubmit(onSkillCategorySubmit)} className="grid gap-4 py-4"><ScrollArea className="max-h-[70vh] p-1"><div className="grid gap-4 p-3"><div><Label htmlFor="category_name">Category Name</Label><Input id="category_name" {...skillCategoryForm.register("category_name")} />{skillCategoryForm.formState.errors.category_name && <p className="text-destructive text-sm mt-1">{skillCategoryForm.formState.errors.category_name.message}</p>}</div><div><Label htmlFor="cat_icon_image_url">Icon Image URL (Optional)</Label><Input id="cat_icon_image_url" {...skillCategoryForm.register("icon_image_url")} placeholder="https://..." />{skillCategoryForm.formState.errors.icon_image_url && <p className="text-destructive text-sm mt-1">{skillCategoryForm.formState.errors.icon_image_url.message}</p>}{skillCategoryForm.watch("icon_image_url") && (<div className="mt-2 flex items-center gap-2"><span className="text-xs">Preview:</span><IconPreview url={skillCategoryForm.watch("icon_image_url")} DefaultIcon={TagIcon}/></div>)}</div><div><Label htmlFor="cat_sort_order">Sort Order</Label><Input id="cat_sort_order" type="number" {...skillCategoryForm.register("sort_order")} /></div></div></ScrollArea><DialogFooter><DialogClose asChild><Button type="button" variant="outline">Cancel</Button></DialogClose><Button type="submit">{currentSkillCategory ? 'Save Changes' : 'Add Category'}</Button></DialogFooter></form></DialogContent></Dialog>
      <AlertDialog open={showSkillCategoryDeleteConfirm} onOpenChange={setShowSkillCategoryDeleteConfirm}><AlertDialogContent className="bg-destructive border-destructive text-destructive-foreground"><AlertDialogHeader><AlertDialogTitle className="text-destructive-foreground">Delete Category: {skillCategoryToDelete?.category_name}?</AlertDialogTitle><AlertDialogDescription className="text-destructive-foreground/90">This will delete the category and all skills within it. This action cannot be undone.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel onClick={() => { setSkillCategoryToDelete(null); setShowSkillCategoryDeleteConfirm(false);}} className={cn(buttonVariants({ variant: "outline" }), "border-destructive-foreground/40 text-destructive-foreground hover:bg-destructive-foreground/10 hover:text-destructive-foreground hover:border-destructive-foreground/60")}>Cancel</AlertDialogCancel><AlertDialogAction onClick={handleDeleteSkillCategory} className={cn(buttonVariants({ variant: "default" }), "bg-destructive-foreground text-destructive hover:bg-destructive-foreground/90")}>Delete Category</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
      
      {/* Skill Modal & Delete Dialog */}
      <Dialog open={isSkillModalOpen} onOpenChange={(isOpen) => { setIsSkillModalOpen(isOpen); if (!isOpen) { setCurrentSkill(null); setParentKeySkillCategoryId(null); } }}><DialogContent className="sm:max-w-[425px]"><DialogHeader><DialogTitle>{currentSkill ? 'Edit Skill' : 'Add New Skill'}</DialogTitle></DialogHeader><form onSubmit={skillForm.handleSubmit(onSkillSubmit)} className="grid gap-4 py-4"><ScrollArea className="max-h-[70vh] p-1"><div className="grid gap-4 p-3"><Input type="hidden" {...skillForm.register("category_id")} /><div><Label htmlFor="skill_name">Skill Name</Label><Input id="skill_name" {...skillForm.register("skill_name")} />{skillForm.formState.errors.skill_name && <p className="text-destructive text-sm mt-1">{skillForm.formState.errors.skill_name.message}</p>}</div></div></ScrollArea><DialogFooter><DialogClose asChild><Button type="button" variant="outline">Cancel</Button></DialogClose><Button type="submit">{currentSkill ? 'Save Changes' : 'Add Skill'}</Button></DialogFooter></form></DialogContent></Dialog>
      <AlertDialog open={showSkillDeleteConfirm} onOpenChange={setShowSkillDeleteConfirm}><AlertDialogContent className="bg-destructive border-destructive text-destructive-foreground"><AlertDialogHeader><AlertDialogTitle className="text-destructive-foreground">Delete Skill: {skillToDelete?.skill_name}?</AlertDialogTitle><AlertDialogDescription className="text-destructive-foreground/90">This action cannot be undone.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel onClick={() => { setSkillToDelete(null); setShowSkillDeleteConfirm(false);}} className={cn(buttonVariants({ variant: "outline" }), "border-destructive-foreground/40 text-destructive-foreground hover:bg-destructive-foreground/10 hover:text-destructive-foreground hover:border-destructive-foreground/60")}>Cancel</AlertDialogCancel><AlertDialogAction onClick={handleDeleteSkill} className={cn(buttonVariants({ variant: "default" }), "bg-destructive-foreground text-destructive hover:bg-destructive-foreground/90")}>Delete Skill</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>

      {/* Language Modal & Delete Dialog */}
      <Dialog open={isLanguageModalOpen} onOpenChange={(isOpen) => { setIsLanguageModalOpen(isOpen); if (!isOpen) setCurrentLanguage(null); }}><DialogContent className="sm:max-w-[425px]"><DialogHeader><DialogTitle>{currentLanguage ? 'Edit Language' : 'Add New Language'}</DialogTitle></DialogHeader><form onSubmit={languageForm.handleSubmit(onLanguageSubmit)} className="grid gap-4 py-4"><ScrollArea className="max-h-[70vh] p-1"><div className="grid gap-4 p-3"><div><Label htmlFor="language_name">Language Name</Label><Input id="language_name" {...languageForm.register("language_name")} />{languageForm.formState.errors.language_name && <p className="text-destructive text-sm mt-1">{languageForm.formState.errors.language_name.message}</p>}</div><div><Label htmlFor="proficiency">Proficiency (e.g., Native, Fluent, Conversational)</Label><Input id="proficiency" {...languageForm.register("proficiency")} /></div><div><Label htmlFor="lang_icon_image_url">Icon Image URL (Optional)</Label><Input id="lang_icon_image_url" {...languageForm.register("icon_image_url")} placeholder="https://..." />{languageForm.formState.errors.icon_image_url && <p className="text-destructive text-sm mt-1">{languageForm.formState.errors.icon_image_url.message}</p>}{languageForm.watch("icon_image_url") && (<div className="mt-2 flex items-center gap-2"><span className="text-xs">Preview:</span><IconPreview url={languageForm.watch("icon_image_url")} DefaultIcon={LanguagesIcon}/></div>)}</div><div><Label htmlFor="lang_sort_order">Sort Order</Label><Input id="lang_sort_order" type="number" {...languageForm.register("sort_order")} /></div></div></ScrollArea><DialogFooter><DialogClose asChild><Button type="button" variant="outline">Cancel</Button></DialogClose><Button type="submit">{currentLanguage ? 'Save Changes' : 'Add Language'}</Button></DialogFooter></form></DialogContent></Dialog>
      <AlertDialog open={showLanguageDeleteConfirm} onOpenChange={setShowLanguageDeleteConfirm}><AlertDialogContent className="bg-destructive border-destructive text-destructive-foreground"><AlertDialogHeader><AlertDialogTitle className="text-destructive-foreground">Delete Language: {languageToDelete?.language_name}?</AlertDialogTitle><AlertDialogDescription className="text-destructive-foreground/90">This action cannot be undone.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel onClick={() => { setLanguageToDelete(null); setShowLanguageDeleteConfirm(false);}} className={cn(buttonVariants({ variant: "outline" }), "border-destructive-foreground/40 text-destructive-foreground hover:bg-destructive-foreground/10 hover:text-destructive-foreground hover:border-destructive-foreground/60")}>Cancel</AlertDialogCancel><AlertDialogAction onClick={handleDeleteLanguage} className={cn(buttonVariants({ variant: "default" }), "bg-destructive-foreground text-destructive hover:bg-destructive-foreground/90")}>Delete Language</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
    </>
  );
}
